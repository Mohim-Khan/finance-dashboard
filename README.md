...
**DAX measures built:** Total Amount, Total Transactions, Average Transaction, Total Fee, Total Tax, plus the Dynamic Metric switch measure driving the KPI cards and charts from a single slicer.
...
- **Fraud is rare but sharply distinguishable**: only ~1.3% of transactions are flagged as fraud, but flagged transactions carry a dramatically higher average risk score (~83) than clean ones (~35) — meaning the existing risk score is doing real work and could support an automated pre-screening threshold.
